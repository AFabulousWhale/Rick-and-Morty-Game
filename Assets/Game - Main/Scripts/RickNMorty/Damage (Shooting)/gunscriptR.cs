...
public class gunscriptR : MonoBehaviour
{
    ...
    {
        currentAmmo = maxAmmo;
        GameObject.Find("BulletR").gameObject.GetComponent<Renderer>().enabled = true;
    }
    ...
    void Update()
    {
        if (_ChangingCharacter.charSelect == 0)
        {
            ...
            if (Input.GetButton("Fire1") && (Time.time >= nextTimeToFire) && (!_AbilitiesMain.Teleporting) && (!_AbilitiesMain.Drinking))
            {
        ...
        {
            GameObject.Find("BulletR").gameObject.GetComponent<Renderer>().enabled = false;
        }
    }
    ...
    public void FireBullets()
    {
        ...
        Bullets.transform.rotation = BulletSpawn.transform.rotation;
